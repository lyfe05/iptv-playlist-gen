...
EPG_URL = "https://iptv-org.github.io/epg/guide.xml"
LOGO_BASE_URL = "https://iptv-org.github.io/logos/"
WIKIPEDIA_API_URL = "https://en.wikipedia.org/w/api.php"
PLAYLIST_STORAGE = "/tmp/playlists.json"
...
def load_playlist_urls() -> List[str]:
    """Load playlist URLs from a JSON file or environment variable."""
    env_urls = os.getenv("PLAYLIST_URLS", "")
    if env_urls:
...
async def fetch_logo(channel_name: str, tvg_id: str) -> str:
    ...
    cache_file = f"/tmp/logo_{channel_name}.json"
                                        ...
                                        async with session.get(WIKIPEDIA_API_URL, params=params) as url_response:
                                            if url_response.status == 200:
                                                ...
                                                for img_page_id, img_page in img_pages.items():
                                                    ...
                                                    logo_url = img_info.get("url", "")
                                                    try:
                                                        with open(cache_file, "w") as f:
                                                            json.dump({"url": logo_url}, f)
                                                    except:
                                                        pass
                                                    return logo_url
    except Exception as e:
    ...
    logo_url = f"{LOGO_BASE_URL}{tvg_id}.png" if tvg_id else ""
    try:
        with open(cache_file, "w") as f:
            json.dump({"url": logo_url}, f)
    except:
...
async def sync_playlists(playlist_urls: List[str]):
    ...
    for url in playlist_urls:  # Sequential to avoid timeout
...
async def add_playlists():
    ...
    try:
        ...
        logger.error(f"Error in add_playlists: {str(e)}")
